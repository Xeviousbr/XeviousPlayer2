...
namespace VirtualSoftware
{
    ...
    public partial class ColorSlider : Control
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            if (!Enabled)
            {
                Color[] desaturatedColors = UnsaturateColors(thumbOuterColor, thumbInnerColor, thumbPenColor,
        ...
        /// <returns></returns>
        public static Color[] UnsaturateColors(params Color[] colorsToUnsaturate)
        {
            Color[] colorsToReturn = new Color[colorsToUnsaturate.Length];
            for (int i = 0; i < colorsToUnsaturate.Length; i++)
            {
                //use NTSC weighted avarage
                int gray =
                    (int)(colorsToUnsaturate[i].R * 0.3 + colorsToUnsaturate[i].G * 0.6 + colorsToUnsaturate[i].B * 0.1);
